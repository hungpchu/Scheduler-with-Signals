...
void update_run_time(thread_info_t *info) {
        /* TODO: implement this function */
         clock_gettime(CLOCKID,&info->suspend_time);
...
void update_wait_time(thread_info_t *info) {
        /* TODO: implement this function */
       clock_gettime(CLOCKID,&info->resume_time);
...
static void init_sched_queue(int queue_size)
{
	...
	timer_create(CLOCKID, &sev, &timer);
...
void setup_sig_handlers() {
	...
	sa1.sa_sigaction = &cancel_thread;
	...
	sa2.sa_sigaction = &suspend_thread;
...
{
	new_its.it_interval.tv_sec = QUANTUM;
    	new_its.it_interval.tv_nsec = 0;
    	new_its.it_value.tv_sec = QUANTUM;
    	new_its.it_value.tv_nsec = 0;

	wait_for_queue();

	/* TODO: start the timer */
	timer_settime(timer, 0, &new_its, NULL);
